...
export default function DoctorDashboard() {
  ...
  // Fetch doctor data for clinic info (optional, for future use)
  useEffect(() => {
  ...
  // Get current doctor info from fetched data (if available) or use user info
  const currentDoctor = doctors.find((d) => d.isActive) || doctors[0];
  const doctorSpecialization =
    currentDoctor?.specialization || "General Practitioner";
  const doctorClinic = currentDoctor?.clinic || "Central Health Clinic";
  const doctorSchedule = currentDoctor?.defaultSchedule || "09:00 - 12:00";
