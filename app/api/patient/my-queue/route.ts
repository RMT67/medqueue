  ...
  try {
    ...
    const queues = await Promise.all(
      bookings.map(async (booking) => {
        ...
        console.log(`[my-queue API] ====== Processing booking ${booking.bookingNumber || booking.queueNumber || booking._id} ======`)
        ...
        if (appointmentTime) {
          try {
            ...
            const formatted = `${localHours.toString().padStart(2, "0")}:${localMinutes.toString().padStart(2, "0")}`
          ...
          console.warn("[my-queue API] ⚠️ appointmentTime is NULL or UNDEFINED!")
        }

        // #region agent log
        fetch('http://127.0.0.1:7242/ingest/12920fd2-3179-4444-a1d4-b2f4f8633224',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'my-queue/route.ts:180',message:'API: Returning queue data',data:{bookingId:booking._id?.toString(),appointmentTime:appointmentTime?.toString(),timeRange:scheduleTimeRange,scheduleDate},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'C'})}).catch(()=>{});
        // #endregion
        return {
